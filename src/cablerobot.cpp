...
void CableRobot::GoToCenter()
{
    ...
    if (internalDelayCounter>30) {
        internalDelayCounter = 0;
        for (int i=0; i<numberOfActuators; i+=2) servoMotor[i].SetMaxTorque();
    }
    if (homingFlag) {
        cout << "here2" << endl;
        homingStateFlags = switchActuatedCable;
        for (int i=0; i<numberOfActuators; i+=2) servoMotor[i].SetStartingWinchParameter();
        internalDelayCounter = 0;
        homingActuator-= 2;
        //Send Start Values
...
void CableRobot::SwitchActuatedCable()
{
    ...
    if (homingFlag && internalDelayCounter == 0) {
        ...
        uint8_t flag = 1;
        for (int i=0; i<numberOfActuators; i+=2) {
            if (i==homingActuator && servoMotor[i].servoMotorOperationState==GoldSoloWhistleDrive::cyclicPosition)
                flag *=1;
            else if (i!=homingActuator && servoMotor[i].servoMotorOperationState==GoldSoloWhistleDrive::cyclicTorque)
                flag *=1;
            else
                flag *=0;
        }
        if (flag) {
            ...
            servoMotor[homingActuator].SetPoly7IncrementalParameters(-incrementalHomingLength,transitionTime);
            internalDelayCounter = 0;
            measurementStage = 0;
        }
    }
...
{
    cout << servoMotor[0].pulleyAngle << '\t' << servoMotor[2].pulleyAngle << '\t' << servoMotor[4].pulleyAngle << endl;
...
void CableRobot::IdleActuatorPvt33Fun()
{
    ...
    if (actuatorPvt33ProcessFinished) {
        //emit SendData(servoMotor[0].pulleyAngle,servoMotor[1].pulleyAngle,servoMotor[2].pulleyAngle);
    }
}
...
void CableRobot::MoveActuatorPvt33()
{
    if (enablePvt && robotGloballyEnabled>=numberOfActuators) {
        if (pvtCounter<numberOfPvt33Data) {
            pvtCounter++;
            emit SendData(servoMotor[0].pulleyAngle,servoMotor[2].pulleyAngle,servoMotor[4].pulleyAngle);
            for (int i=0; i<numberOfActuators; i+=2) {
            ...
            for (int i=0; i<numberOfActuators; i+=2) servoMotor[i].SetPoly7GoStartParameters(goHomeTime);
        }
    } else {
...
void CableRobot::CollectEnableRequest(int enable)
{
    ...
    for (int i=0;i<numberOfActuators;i++) {
        if (enable) {
            if (servoMotor[i].servoMotorState != GoldSoloWhistleDrive::operationEnabled) servoMotor[i].Enable();
        } else {
...
{
    for (int i=0;i<numberOfActuators;i++) {
        if (servoMotor[i].servoMotorState == GoldSoloWhistleDrive::fault) servoMotor[i].FaultReset();
    }
...
void CableRobot::CollectMeasurementRequest()
{
    measurementFlag = 1;
}
...
{
    for (int i=0; i<numberOfActuators; i+=2) {
        servoMotor[i].SetHomeWinchParameters(theData[i/2],theData[i/2+numberOfActuators/2],theData[i/2+numberOfActuators]);
