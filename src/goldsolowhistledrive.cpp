...
{
    if (inputPdos.statusWord[offStatusBit] == set) { // drive idle
        state = switchOnDisabled;
    } else {
        if (inputPdos.statusWord[onStatusBit] == set) { // drive operational or in operational progress
        ...
        } else { // drive in a stop or fault
            if (inputPdos.statusWord[faultStatusBit] == reset) { // drive in quick stop
...
{
    switch (stateFlags) {
    case switchOnDisabled : {// We previously asked for a state change: it occurred
        cout << "Drive " << position << " Status Word: " << inputPdos.statusWord << endl;
        cout << "Drive " << position << " Idle." << endl;
        stateFlags = nullState;
        break;
    }
    case readyToSwitchOn : { // We are starting the enabling sequence, transition 2
        cout << "Drive " << position << " Status Word: " << inputPdos.statusWord << endl;
...
{
    switch (stateFlags) {
    case readyToSwitchOn : {// We previously asked for a feasible state change, now we ask for another
        cout << "Drive " << position << " Status Word: " << inputPdos.statusWord << endl;
...
{
    switch (stateFlags) {
    case switchOn : { // We previously asked for a feasible state change, now we ask for another
        cout << "Drive " << position << " Status Word: " << inputPdos.statusWord << endl;
...
{
    switch (stateFlags) {
    case operationEnabled : { // We previously asked for a state change: it occurred
        cout << "Drive " << position << " Status Word: " << inputPdos.statusWord << endl;
    ...
    case switchOnDisabled : { // We want to disable the drive
        cout << "Drive " << position << " Status Word: " << inputPdos.statusWord << endl;
...
{
    switch (stateFlags) {
    case switchOnDisabled : { // we are requesting a fault reset
        cout << "Drive " << position << " Status Word: " << inputPdos.statusWord << endl;
        cout << "Drive " << position << " requesting Fault Reset: going Idle" << endl;
        outputPdos.controlWord.reset();
    ...
    default : {
        cout << "Drive " << position << " Status Word: " << inputPdos.statusWord << endl;
        cout << "Drive " << position << " encountered an error asking for state change code: " << stateFlags << endl;
        stateFlags = nullState;
        break;
    }
    }
...
GoldSoloWhistleDrive::GoldSoloWhistleDrive(uint8_t thisSlavePosition)
{
    ...
    */
    ...
    domainRegisters[0] = {alias, position, vendor_id, product_code, controlWordIndex, controlWordSubIndex, &offsetOut.controlWord,NULL};
    domainRegisters[1] = {alias, position, vendor_id, product_code, modesOfOperationIndex, modesOfOperationSubIndex, &offsetOut.modesOfOperation,NULL};
    domainRegisters[2] = {alias, position, vendor_id, product_code, targetTorqueIndex, targetTorqueSubIndex, &offsetOut.targetTorque,NULL};
    domainRegisters[3] = {alias, position, vendor_id, product_code, targetPositionIndex, targetPositionSubIndex, &offsetOut.targetPosition,NULL};
    domainRegisters[4] = {alias, position, vendor_id, product_code, targetVelocityIndex, targetVelocitySubIndex, &offsetOut.targetVelocity,NULL};
    domainRegisters[5] = {alias, position, vendor_id, product_code, statusWordIndex, statusWordSubIndex, &offsetIn.statusWord,NULL};
    domainRegisters[6] = {alias, position, vendor_id, product_code, modesOfOperationDisplayIndex, modesOfOperationDisplaySubIndex, &offsetIn.modesOfOperationDisplay,NULL};
    domainRegisters[7] = {alias, position, vendor_id, product_code, positionActualValueIndex, positionActualValueSubIndex, &offsetIn.positionActualValue,NULL};
    domainRegisters[8] = {alias, position, vendor_id, product_code, velocityActualValueIndex, velocityActualValueSubIndex, &offsetIn.velocityActualValue,NULL};
    domainRegisters[9] = {alias, position, vendor_id, product_code, torqueActualValueIndex, torqueActualValueSubIndex, &offsetIn.torqueActualValue,NULL};
    domainRegisters[10] = {alias, position, vendor_id, product_code, digitalInputsIndex, digitalInputsSubIndex, &offsetIn.digitalInputs,NULL};
    domainRegisters[11] = {alias, position, vendor_id, product_code, auxiliaryPositionActualValueIndex, auxiliaryPositionActualValueSubIndex, &offsetIn.auxiliaryPositionActualValue,NULL};
...
int GoldSoloWhistleDrive::SdoRequests(ec_sdo_request_t *sdoPointer, ec_slave_config_t *configPointer)
{
    if (!(sdoPointer = ecrt_slave_config_create_sdo_request(configPointer, modesOfOperationIndex, modesOfOperationSubIndex,cyclicPosition)))
        {
        cout << "Failed to create SDO request." << endl;
        return 1;
        }
    ecrt_sdo_request_timeout(sdoPointer, 500);
    ecrt_slave_config_sdo8(configPointer, modesOfOperationIndex, modesOfOperationSubIndex,cyclicPosition);
    if (!(sdoPointer = ecrt_slave_config_create_sdo_request(configPointer, homingMethodIndex, homingMethodSubIndex, homingOnPositionMethod)))
        {
        cout << "Failed to create SDO request." << endl;
        return 1;
        }
    ecrt_sdo_request_timeout(sdoPointer, 500);
    ecrt_slave_config_sdo8(configPointer,homingMethodIndex, homingMethodSubIndex, homingOnPositionMethod);
    return 0;
}

void GoldSoloWhistleDrive::LoopFunction()
{
...
void GoldSoloWhistleDrive::ReadInputs()
{
    ...
    inputPdos.modesOfOperationDisplay = EC_READ_S8(domainDataPointer + offsetIn.modesOfOperationDisplay);
    DetermineState();
    inputPdos.positionActualValue = EC_READ_S32(domainDataPointer + offsetIn.positionActualValue);
    inputPdos.velocityActualValue = EC_READ_S32(domainDataPointer + offsetIn.velocityActualValue);
    inputPdos.torqueActualValue = EC_READ_S16(domainDataPointer + offsetIn.torqueActualValue);
    inputPdos.digitalInputs = EC_READ_U32(domainDataPointer + offsetIn.digitalInputs);
    inputPdos.auxiliaryPositionActualValue = EC_READ_S32(domainDataPointer + offsetIn.auxiliaryPositionActualValue);
    (this->*stateManager[state])();
}

void GoldSoloWhistleDrive::WriteOutputs()
{
    EC_WRITE_U16(domainDataPointer + offsetOut.controlWord, static_cast<unsigned short>(outputPdos.controlWord.to_ulong()));
    EC_WRITE_S8(domainDataPointer + offsetOut.modesOfOperation, outputPdos.modesOfOperation);
