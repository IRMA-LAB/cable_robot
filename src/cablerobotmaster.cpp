...
void CableRobotMaster::IdleTransition()
{
  ...
  {
    state = stateFlags;
    for (int i = 0; i < GoldSoloWhistleNumber; i++)
      goldSoloWhistleSlave[i].SetTargetDefaults();
  }
}
...
CableRobotMaster::CableRobotMaster(QObject* parent)
  : QObject(parent), cableRobot(nullptr, goldSoloWhistleSlave)
{
  ...
  */
  ...
  for (int i = 0; i < numberOfSlaves; i++)
    domainElementsNumber += etherCatSlave[i]->numberOfDomainEntries;
...
void CableRobotMaster::LoopFunction()
{
  for (int i = 0; i < slavesNumber; i++)
    etherCatSlave[i]->ReadInputs(); // Read pdos, act accordingly
  (this->*stateManager[state])();   // check if the state has to be changed
  (this->*stateMachine[state])();   // do your job
  for (int i = 0; i < slavesNumber; i++)
    etherCatSlave[i]->WriteOutputs(); // Write all the necessary pdos
}
