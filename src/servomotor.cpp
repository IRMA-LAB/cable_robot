...
void ServoMotor::UpdateState()
{
    servoMotorState = thisDrive->state;
    servoMotorOperationState = thisDrive->operationState;
    cableLength = cableHomeLength+thisWinch.FromCountsToLength(thisDrive->inputPdos.positionActualValue-homeMotorCounts);
    pulleyAngle = pulleyHomeAngle+thisWinch.FromCountsToPulleyAngleRad(thisDrive->inputPdos.auxiliaryPositionActualValue-homePulleyCounts);
}
...
void ServoMotor::LoopFunction()
{
    if (servoMotorState == GoldSoloWhistleDrive::operationEnabled) {
        ...
        case GoldSoloWhistleDrive::cyclicPosition : {
            if (motionStatus == set) {
                if (upCounter>=10000) upCounter = 10000;
                if (downCounter<=0) downCounter =0;
                switch (typeOfMotion) {
                case PositionMovePlus : {
                    thisDrive->outputPdos.TargetPosition += thisWinch.FromLengthToCounts((double)upCounter*0.0000001);
                    break;
                }
                ...
                case PositionMicroMovePlus : {
                    thisDrive->outputPdos.TargetPosition += thisWinch.FromLengthToCounts((double)upCounter*0.000000001);
                    break;
                }
                ...
                if (thisDrive->inputPdos.torqueActualValue>10) thisDrive->outputPdos.TargetPosition -= (int)ceil(abs((double)thisDrive->inputPdos.velocityActualValue)*0.00005);
                if (thisDrive->inputPdos.torqueActualValue<-10) thisDrive->outputPdos.TargetPosition += (int)ceil(abs((double)thisDrive->inputPdos.velocityActualValue)*0.00005);
            }
            break;
        }
        case GoldSoloWhistleDrive::cyclicVelocity : {
            if (motionStatus == set) {
                ...
                if (thisDrive->inputPdos.velocityActualValue>2000) thisDrive->outputPdos.TargetVelocity -= thisDrive->outputPdos.TargetVelocity/100;
                else if (thisDrive->inputPdos.velocityActualValue<-2000) thisDrive->outputPdos.TargetVelocity += -thisDrive->outputPdos.TargetVelocity/100;
            }
            break;
        }
        case GoldSoloWhistleDrive::cyclicTorque : {
            if (upCounter > 30) {
                ...
                if (motionStatus == set) {
                    ...
                    if (thisDrive->inputPdos.torqueActualValue>10) thisDrive->outputPdos.TargetTorque -= thisDrive->outputPdos.TargetTorque/10;
                    else if (thisDrive->inputPdos.torqueActualValue<-10) thisDrive->outputPdos.TargetTorque += -thisDrive->outputPdos.TargetTorque/10;
                }
            }
            break;
        }
...
void ServoMotor::ChangeOperationMode(int theMode)
{
    upCounter = 0;
    downCounter = 10000;
    thisDrive->operationStateFlags = (GoldSoloWhistleDrive::GoldSoloWhistleOperationState)theMode;
}
...
void ServoMotor::SetHomeWinchParameters(double theCable, double thePulley, double theCable2)
{
    cableHomeLength = theCable;
    pulleyHomeAngle = thePulley;
    cableEffectiveHomeLength = theCable2;
    startMotorCounts = homeMotorCounts+thisWinch.FromLengthToCounts(cableEffectiveHomeLength-cableHomeLength);
}

void ServoMotor::SetMaxTorque()
{
    if (abs(thisDrive->inputPdos.torqueActualValue) < maxTorque) thisDrive->outputPdos.TargetTorque--;
}
...
{
    thisDrive->outputPdos.TargetPosition = startMotorCounts+thisWinch.FromLengthToCounts(thePosition);
...
void ServoMotor::MovePoly7Incremental(double t)
{
    ...
    if (normalizedTime<=1.0 && poly7Flag) {
        thisDrive->outputPdos.TargetPosition = (int)(startCounts+(stopCounts-startCounts)*(poly7Coeff[0]*pow(normalizedTime,4.0)+poly7Coeff[1]*pow(normalizedTime,5.0)+poly7Coeff[2]*pow(normalizedTime,6.0)+poly7Coeff[3]*pow(normalizedTime,7.0)));
    } else {
        poly7Flag = 0;
    }
}
