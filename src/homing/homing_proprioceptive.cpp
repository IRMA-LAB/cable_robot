...
STATE_DEFINE(HomingProprioceptive, Optimizing, NoEventData)
{
  ...
  const std::string script_loc = std::string(SRCDIR) + "matlab/homing/ExternalHomingScript.m";
  RunMatlabScript(script_loc);
  ...
  const QString results_location = QString(SRCDIR) + "matlab/homing/results.json";
  if (ParseExtFile(results_location, home_data))
  {
...
bool HomingProprioceptive::ParseExtFile(const QString& filepath,
