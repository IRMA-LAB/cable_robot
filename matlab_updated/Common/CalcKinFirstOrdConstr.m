function [constr,cdpr_v] = CalcKinFirstOrdConstr(velocity,orientation_d,l_d,cdpr_p,cdpr_v)
%CALCKINFIRSTORDCONSTR updates time dependent variables connected to the 1th  
%   order inverse kinematics problem of the cdpr and evaluates kinematic constraints. . 
%  
...
for i=1:length(cdpr_v.cable)
  ...
  cdpr_v.geometric_jacobian_d(i,:) = cdpr_v.cable(i).geometric_jacobian_row_d;
  cdpr_v.analitic_jacobian_d(i,:) = cdpr_v.cable(i).analitic_jacobian_row_d;
  cdpr_v.cable(i).speed = l_d(i);
  constr(i,1) = dot(cdpr_v.cable(i).vers_rho,cdpr_v.cable(i).vel_OA_glob)-cdpr_v.cable(i).speed;
end
