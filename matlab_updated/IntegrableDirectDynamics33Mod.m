function vect = IntegrableDirectDynamics33Mod(cdpr_p,cdpr_v,ut,...
        spline_s,time,state)
  
    for i=1:cdpr_p.n_cables
%         if time >0
%             [l(i,1),l_d(i,1),l_d2(i,1)] = Interpolate(spline_s.l(i),time,ut.t_interval);
%         else
%             [l(i,1),l_d(i,1),l_d2(i,1)] = Interpolate(spline_s.l(i),time,ut.t_interval);
%         end
    ...
    [K0,cdpr_v] = CalcKinZeroOrdConstr(state(1:3),state(4:6),l,cdpr_p,cdpr_v);
    [K1,cdpr_v] = CalcKinFirstOrdConstr(state(7:9),state(10:12),l_d,cdpr_p,cdpr_v);
