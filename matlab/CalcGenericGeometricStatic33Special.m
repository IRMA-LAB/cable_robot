function vector = CalcGenericGeometricStatic33Special(cdpr_p,record,parameters,variables,mask)
...
for i=1:cdpr_p.pose_dim
  if (mask(i,1) == 1)
    ...
    Wa(actuated_index,1) = cdpr_v.ext_load_ss(i,1);
    actuated_index = actuated_index + 1;
  else
    Ju(:,not_actuated_index) = cdpr_v.analitic_jacobian(:,i);
    Wu(not_actuated_index,1) = cdpr_v.ext_load_ss(i,1);
