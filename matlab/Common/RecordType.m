classdef RecordType
  ...
  methods
    ...
    function obj = SetFigureParameter(obj,par,title)
      ...
      lim_x = [round((min_x-incr_x)*10) round((max_x+incr_x)*10)]./10; l_x = (lim_x(2)-lim_x(1)); tickSpace_x = l_x/10; 
      max_y = max(limits(2,:)); min_y = min(limits(2,:)); incr_y = 0.2*(max_y - min_y);
      lim_y = [round((min_y-incr_y)*10) round((max_y+incr_y)*10)]./10; l_y = (lim_y(2)-lim_y(1)); tickSpace_y = l_y/10; 
      max_z = max(limits(3,:)); min_z = min(limits(3,:)); middle_z = (max_z - min_z)/2;
      max_l = max([lim_x lim_y]); lim_z = [round((middle_z-max_l)*8) round((max_z+0.2)*8)]./10; l_z = (lim_z(2)-lim_z(1));
      tickSpace_z = l_z/10; 
