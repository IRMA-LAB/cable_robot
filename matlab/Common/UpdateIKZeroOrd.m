...
cdpr_v.platform = UpdatePlatformPose(position, orientation,...
  cdpr_p.platform.rotation_parametrization, cdpr_p.platform.pos_G_loc, ...
  cdpr_v.platform);

active_actuators_id = cdpr_p.activeActuatorsId();
for i = 1:length(active_actuators_id)
  cdpr_v.cable(i) = UpdateCableZeroOrd(cdpr_p.cable(active_actuators_id(i)),...
    cdpr_v.platform, cdpr_v.cable(i));
