...
struct MotorStatus
{
   ...
   */
  MotorStatus()
    : motor_status_word(0), id(0), motor_position(0), motor_speed(0), motor_torque(0),
      op_mode(0)
  {}
  /**
  ...
  MotorStatus(const id_t _id, const grabec::GSWDriveInPdos& drive_pdos)
    : motor_status_word(drive_pdos.status_word), id(_id),
      motor_position(drive_pdos.pos_actual_value),
   ...
   * @param[in] status_word Motor status word.
   */
  MotorStatus(const id_t _id, const int8_t _op_mode, const int32_t motor_pos,
              ...
              const std::bitset<16> status_word = 0)
    : motor_status_word(status_word), id(_id), motor_position(motor_pos),
      motor_speed(_motor_speed), motor_torque(_motor_torque), op_mode(_op_mode)
  {}

  std::bitset<16> motor_status_word; /**< Motor status word.*/
  id_t id;                           /**< Motor ID.*/
...
struct ActuatorStatus: WinchStatus
{
   ...
   */
  ActuatorStatus() : pulley_angle(0.0), state(0) {}
  /**
  ...
  ActuatorStatus(const id_t _id, const grabec::GSWDriveInPdos& drive_pdos,
                 ...
                 const uint8_t _state = 0)
    : WinchStatus(_id, drive_pdos, cable_len), pulley_angle(pulley_ang), state(_state)
  {}
  /**
   * @brief ActuatorStatus full implicit constructor.
   * @param[in] _id Motor ID.
   ...
   * @param[in] pulley_ang Swivel pulley angle in radians.
   */
  ActuatorStatus(const WinchStatus& winch_status, const double pulley_ang = 0.0,
                 const uint8_t _state = 0)
    : WinchStatus(winch_status), pulley_angle(pulley_ang), state(_state)
  {}
  /**
  ...
  ActuatorStatus(const id_t _id, const int8_t _op_mode, const int32_t motor_pos,
    ...
    : WinchStatus(_id, _op_mode, motor_pos, _motor_speed, _motor_torque, cable_len,
                  aux_pos),
      pulley_angle(pulley_ang), state(_state)
  {}
