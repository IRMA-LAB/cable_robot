...
private:
    ...
    constexpr static uint8_t numberOfEasyCatStates = 2; // Number of state of easycat state machine, according to how we programmed it
...
public:
    ...
    enum EasyCatState {
        idle = 0,
        updateSlave = 1,
    } internalState, slaveFlags; // state machine utilities
    ...
    ec_pdo_entry_info_t slavePdoEntries[6] = { // ethercat utilities, can be retrieved in the xml config file provided by the vendor
        {0x0005, 0x01, 8}, /* Byte0 */
        {0x0005, 0x02, 8}, /* Byte1 */
        {0x0005, 0x03, 8}, /* Byte2 */
        {0x0006, 0x01, 8}, /* Byte0 */
    ...
    ec_sync_info_t slaveSyncs[3] = { // ethercat utilities, can be retrieved in the xml config file provided by the vendor
    ...
    // The last 3 array can be obtained by command line, executing sudo /opt/etherlab/bin/./ethercat cstruct
