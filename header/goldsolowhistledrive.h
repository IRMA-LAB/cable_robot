...
private:
    ...
    struct OffsetOut { // Useful ethercat struct
    ...
    struct OffsetIn { // Useful ethercat struct
    ...
    typedef void (GoldSoloWhistleDrive::*StateFunction)(); // Easyway to implement state machine
...
public:
    ...
    } state, stateFlags; // state machine utilities
    // State machine function array
    StateFunction stateMachine[numberOfGoldSoloWhistleStates] = {&GoldSoloWhistleDrive::SwitchOnDisabledFun,
    ...
    StateFunction stateManager[numberOfGoldSoloWhistleStates] = {&GoldSoloWhistleDrive::SwitchOnDisabledTransitions,
    ...
    ec_pdo_entry_reg_t domainRegisters[GoldSoloWhistleDomainEntries]; // ethercat utilities
    ec_pdo_entry_info_t slavePdoEntries[GoldSoloWhistleDomainEntries] = { // ethercat utilities, can be retrieved in the xml config file provided by the vendor
                                              {controlWordIndex, controlWordSubIndex, 16}, // Start of RxPdo mapping (Outputs)
                                              ...
                                              {statusWordIndex, statusWordSubIndex, 16}, // Start of TxPdo mapping (Inputs)
                                              ...
                                              {auxiliaryPositionActualValueIndex, auxiliaryPositionActualValueSubIndex, 32}
                                            };

    ec_pdo_info_t slavePdos[2] = { // ethercat utilities, can be retrieved in the xml config file provided by the vendor
    ...
    struct OutputPdos { // this is a simple way to store the pdos output values
    ...
    struct InputPdos { // this is a simple way to store the pdos input values
    ...
    enum GoldSoloWhistleOperationState {
        ...
        cyclicVelocity = 9,
        cyclicTorque = 10,
    } operationState, operationStateFlags;

    StateFunction operationStateMachine[numberOfSupportedOperations] = {&GoldSoloWhistleDrive::CyclicPositionFun,
                                                                        &GoldSoloWhistleDrive::CyclicVelocityFun,
                                                                        &GoldSoloWhistleDrive::CyclicTorqueFun};
    // State machine transition function array
    StateFunction operationStateManager[numberOfSupportedOperations] = {&GoldSoloWhistleDrive::CyclicPositionTransition,
